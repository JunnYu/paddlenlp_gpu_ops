...
class LinearCrossEntropyLoss(nn.Linear):  # an instance of nn.Linear to be identified as such
    ...
    * Speed-ups over a compiled paddle baseline only materialize in float 16 with sufficiently large vocabulary sizes /
      numbers of classes or very long sequences or batch sizes.
    * This module is an instance of `nn.Linear` to pick up initialization calls to `nn.Linear`, but the weight matrix
      is transposed compared to normal `nn.Linear` layers.
    * This function will call a (substantial) triton autotune list the first time it is called. You can reduce or
      change the number of evaluated configs by modifying `linear_cross_entropy.fwd_configs` and
      `linear_cross_entropy.bwd_configs`.
    * Be careful when auto-casting this module. Right now, the code default to auto-casting to `float16`. This might
      not be what you need.

    Note (Monitoring): Setting `LinearCrossEntropyLoss(4096, 16384).monitoring = True` will additionally accumulate a
    number of monitoring variables as a dictionary in `module.metrics`. These are
    * Logit Norm
    * Maximal logit value
    * Logit Entropy
    * z-regularization value.
    ...
    Args:
        ...
        N_chunk_size:
            How fine to chunk the leading dimensions. The peak memory load will be N_chunk_size x out_features x 4
            bytes [Default 4096]
        init_method: This is an optional callable initialization function [Default: None]

    Shape:
        - Input x (input embeddings): :math:`(*, H_{in})` where :math:`*` means any number of dimensions including none
    ...
    Attributes:
        weight: the learnable weights of the module of shape
            :math:`(\text{out\_features}, \text{in\_features})`. The values are initialized from
            :math:`\mathcal{U}(-\sqrt{k}, \sqrt{k})`, where :math:`k = \frac{1}{\text{in\_features}}`
        bias:   the learnable bias of the module of shape :math:`(\text{out\_features})`.
                If :attr:`bias` is ``True``, the values are initialized from :math:`\mathcal{U}(-\sqrt{k}, \sqrt{k})`
                where :math:`k = \frac{1}{\text{in\_features}}`

    Examples::

        >>> import paddle >>> from linear_cross_entropy import LinearCrossEntropyLoss >>> module =
        LinearCrossEntropyLoss(4096, 16384) >>> x = paddle.randn([4, 512, 4096]) >>> y = paddle.randn([4, 512],
        dtype=paddle.float32).cast(paddle.int64) >>> loss = module(x, y)
    """
